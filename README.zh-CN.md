...
首次运行时 SceneKeys 会主动询问，之后每当连接到它不认识的键盘时也会询问。内置的
可搜索预设库可以一次填好十二个键，之后任何一个键都能再改。

**已在真机上核对过的键盘：** 华为 MateBook X、罗技 K780、罗技 MX Keys Mini。
**来自厂商文档：** 联想 ThinkPad X1 Carbon Gen 12、2017 年后的 ThinkPad、Apple
Magic Keyboard。其他键盘手动设置一次即可，大约一分钟。预设库里没有任何一条是猜的。
...
一个 F 键时很有用。SceneKeys 内置了约 50 个常用应用的 F 键功能，新添加的应用一开始
就是对的。桌面有单独的一行，与文件资源管理器窗口分开；不想看到的应用可以隐藏。

**中英文界面**（简体中文 / English），可在设置中选择，也可以跟随 Windows。首次运行
会打开一份简短的使用指南，之后随时可以从通知区域菜单再次打开。
...
**任何键盘都能用吗？** 可以。预设库里的键盘一键设置；其他键盘手动逐键设置一次即可。
